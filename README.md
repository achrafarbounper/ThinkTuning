...
- **MCP** : `POST /mcp/sse` renvoie la trace dans le champ `thinking` du résultat
  JSON de `tools/call orchestrate` et, avec `stream: true` (ou dès que
  `enable_thinking: true` est activé), la diffuse aussi progressivement dans
  des événements `orchestrate.thinking` portant
