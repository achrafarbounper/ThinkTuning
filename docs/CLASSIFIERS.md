...
```
   ...
   └───────────┬────────────┘
        ┌──────┴──────────┐
        │  Caches          │  app/application/prediction_result_cache.py (LRU + TTL)
        │  app.application.inference_executor / dynamic_batcher / model_warmup
        │  app.application.onnx_exporter (ONNX Runtime, optionnel)
        └─────────────────┘
```
...
Géré par `app/infrastructure/persistence/model_versioning.py` (la version active est résolue par
`app/application/predictor_cache`).

### Intention — `experiments/intent_models/`
Géré par **`app/infrastructure/persistence/intent_store.py`** (Phase 4) :
...
| Module | Rôle |
|---|---|
| `app/application/prediction_result_cache.py` | Cache de résultats LRU + TTL (par clé normalisée texte+classifieur). |
| `app/application/classifier_registry.py` | Registre singleton de classifieurs (`get_or_create` paresseux). |
...
### Runner (`app/application/intent_trainer.py`)
...
sentiment (`app/application/trainer_runner.py`) : étapes canoniques
`INTENT_TRAIN_JOB_STEPS` (`app/domain/entities/models.py`, alignées sur
...
`app/application/job_logs.py`, annulation via `IntentTrainingCancelled` — l'exception est
...
Requête (`IntentTrainRequest`, `app/domain/entities/models.py`) : `dataset_path`,
