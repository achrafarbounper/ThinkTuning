...
| **Long-Running Tasks** | ✅ | `app/infrastructure/mcp/mcp_server_sse.py` (`_stream_orchestrate` : worker bloquant + pont SSE, heartbeat 10 s), `app/infrastructure/mcp/run_sweeper.py` (signal de santé « jobs actifs bloqués » : récolte des runs périmés), **reprise après coupure (MCP 2.3.0, SCRUM-163)** : `_replay_stream` (watermark monotone sans doublon, ordre conservé, `follow` borné) + `resume_cursor.py` (curseur opaque signé, TTL, `Last-Event-ID` natif) | `tests/test_mcp_stream_stability.py`, `tests/test_mcp_resilience.py`, `tests/test_mcp_sse_resume.py` (24 tests de reconnexion) |
