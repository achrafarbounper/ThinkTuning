...
def build_planner_prompt(
    ...
    return (
        ...
        "- « shell » UNIQUEMENT pour exécuter une VRAIE commande simple et "
        "sûre (ex. git status, ls, cat, python script.py, pip list) ; une "
        "commande complexe ou non sûre est à éviter. Une sous-tâche de "
        "rédaction, de structuration ou de mise en forme n'est JAMAIS "
        "« shell ».\n"
        "- STRUCTURATION / MISE EN FORME des résultats (ex. « structure les "
        "résultats en format : titre, entreprise… ») : ne créez PAS de "
        "sous-tâche, la synthèse du superviseur s'en charge ; créez une "
        "sous-tâche « files » uniquement si un fichier doit être produit.\n"
