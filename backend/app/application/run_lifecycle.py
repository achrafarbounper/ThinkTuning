...
valeur que ``app/infrastructure/persistence/audit_store.py`` — ``agent_run`` / ``approval``) pour
éviter
...
from app.infrastructure.persistence.approval_store import APPROVED

# Actions d'audit (alignées sur app/infrastructure/persistence/audit_store.py — voir docstring).
ACT_RUN = "agent_run"
