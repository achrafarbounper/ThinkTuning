...
def _lazy_agent_config() -> Callable[[], Callable[..., dict]]:
    """Résout ``app.application.agent_cache.agent_config`` au premier appel.

    Import paresseux VOLONTAIRE : ``app.application.agent_cache`` tire fastapi/requests
    et la pile agentique — seules ``thinktuning://config`` et
    ``thinktuning://health`` paient ce coût, les autres lectures non.
    """

    def _resolve() -> Callable[..., dict]:
        from app.application.agent_cache import agent_config  # import paresseux (lourd)
...
def _lazy_job_logs() -> Callable[[], Callable[..., dict]]:
    """Résout ``app.application.job_logs.get_logs`` au premier appel (buffer mémoire).
    ...
    def _resolve() -> Callable[..., dict]:
        from app.application.job_logs import get_logs  # import paresseux (léger, stdlib)
...
def _lazy_job_metrics() -> Callable[[], Callable[..., dict]]:
    ...
    Miroir LECTURE SEULE de ``app/infrastructure/persistence/job_store.py::get_job_metrics`` (même
...
def _read_model_version_info(version: str) -> dict:
    ...
    (``model_versions`` — mêmes conventions que
