...
class CapabilityRouter:
    ...
    def list_tools(self, server: str | None = None) -> list[MCPHostTool]:
        return [
            MCPHostTool(
                name=name,
                description=f"Optional MCP capability: {name}",
                input_schema={"type": "object"},
                read_only=name != "git_commit",
            )
    ...
    def call(self, name: str, arguments: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        verdict = decide(
            name, arguments, ActionCategory.WRITE if name == "git_commit" else ActionCategory.READ
        )
