...
persisté dans ``core/flow_store`` (sessions ``agent_flows``) pour apparaître
...
# (``core/flow_store.STATUSES`` — aligné sur ``run_lifecycle.RUN_STATUS_TO_API``).
...
def trace_action_flow(
    ...
    if not _MCP_FLOW_ENABLED:
        return
    try:
        from core.flow_store import COMPLETED as FLOW_COMPLETED
        from core.flow_store import ERROR as FLOW_ERROR
        from core.flow_store import get_flow_store
