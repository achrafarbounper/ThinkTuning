...
Activé par le flag ``AGENT_CONTEXT`` (cf. ``app/application/feature_flags.py``). Trois
briques indépendantes, toutes best-effort et sans dépendance externe :
    ...
    - mémoire inter-sessions : ``format_memory_note`` + helpers SQLite dans
      ``app/infrastructure/persistence/session_store.py`` (table ``agent_memory``), qui permettent à
