...
Décisions de policy intégrées au flux :
    ...
    - ``APPROVE``      → l'action est mise en attente : le run se termine avec
      ...
      d'approbation humaine passe par ``app/infrastructure/persistence/approval_store`` côté
      legacy, ou
...
class AgentCore:
    ...
    def _resolve_authz_gate(self) -> Any | None:
        ...
        if self._authz_enforcer is None:
            from app.application.feature_flags import flag  # import paresseux (anti-cycle)
