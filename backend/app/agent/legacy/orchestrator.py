...
def build_role_agent(
    ...
    if role_name == "lead":
        ...
        required = {
            name: required_args_registry[name] for name in tools if name in required_args_registry
        }
...
class MultiAgentCoordinator:
    ...
    def __init__(
        self,
        llm_client,
        role_builder: Callable[..., Any] | None = None,
        ...
        self._enable_tool_proposals = self._tool_proposals_requested and _dynamic_tools_enabled()
        ...
        try:
            ...
            if results:
                ...
                meta = {
                    ...
                    "engine": str(getattr(self._intent_classifier, "engine", "") or ""),
                }
        except Exception as exc:  # pragma: no cover - chemin défensif
            logger.warning(
                "Classification d'intention échouée (repli « action ») : %s",
                exc,
            )
        logger.info(
    ...
    def _plan(
    ...
    ) -> tuple[list[PlanTask] | None, dict[str, Any] | None, str | None]:
        ...
        raw = None
        try:
            result = lead.run_detailed(planner_prompt, on_thinking=None, on_tool_event=None)
    ...
    def _handle_tool_proposals(
        ...
        for note in notes:
            ...
            code = TOOL_PROPOSAL_LIMITED if "plafond" in reason else TOOL_PROPOSAL_REJECTED
            entry: dict[str, Any] = {
                "name": note.get("name", ""),
    ...
    def _dispatch(
        ...
        for task in tasks:
            decision = intent_decision_for(
                task.role,
                task.intent,
                active=self._intent_active,
            )
            if decision == "ignored":
                skipped[task.task_id] = self._skipped_worker(task)
                logger.info(
                    "agent_decision role=%s task=%s intent=%s confidence=%.2f decision=ignored",
                    task.role,
                    task.task_id,
                    task.intent,
    ...
    def _run_worker(
        ...
        if estimate_tokens(worker_prompt) > self._max_worker_tokens:
            return self._worker_error(
                task,
                TOKEN_BUDGET_EXCEEDED,
                "Budget de jetons dépassé avant exécution.",
                started,
            )
        ...
        effective_rounds: int | None = None
        if self._budget_pool is not None:
            ...
            if granted <= 0:
                return self._worker_error(
                    task,
                    TOKEN_BUDGET_EXCEEDED,
                    "Budget global du run superviseur épuisé (quota d'appels d'outils partagé).",
                    started,
                )
    ...
    def _run_worker_core(
        ...
        resume_request_id: str | None = None,
        on_thinking=None,
        tool_counter: dict[str, int] | None = None,
    ) -> dict[str, Any]:
        thinking_parts: list[str] = []
        try:
            result = agent.run_detailed(
                worker_prompt,
                on_tool_event=tool_hook,
    ...
    def run(
        ...
        try:
            ...
            if tasks is None:
                if (plan_error or {}).get("error_code") == PLAN_EMPTY and (intent_meta or {}).get(
                    "intent"
                ) == "chat":
            ...
            if pending_approvals:
                ...
                for w in pending_approvals:
                    self._remember_resume(
                        str(w.get("request_id")),
                        {
                            **snapshot,
                            "resume_task": next(
            ...
            if workers and all(w.get("intent_decision") == "ignored" for w in workers):
                fsm = fsm.transition(MultiRunState.FALLBACK_CHAT)
                return self._fallback_outcome(
                    str(prompt),
                    tasks,
    ...
    @staticmethod
    def _approval_row(request_id: str) -> dict[str, Any] | None:
        ...
        try:
            from app.infrastructure.persistence.approval_store import (
                get_approval_store,  # import local
            )
    ...
    def _resume(self, resume_request_id: str, on_event, started: float) -> dict[str, Any]:
        ...
        resumed = self._run_worker(
            task,
            self._worker_prompt_for(task, prompt),
            on_event,
            resume_request_id=resume_request_id,
        )
        ...
        workers = [resumed if w.get("task_id") == task.task_id else w for w in workers]
        ...
        fsm = fsm.transition(MultiRunState.SYNTHESIZING)
        final_answer, thinking = self._final_synthesis(prompt, workers, unexecuted, on_event)
        fsm = fsm.transition(MultiRunState.COMPLETED)
