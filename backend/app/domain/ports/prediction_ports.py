...
(``app/application/predictor_cache.py``, ``app/infrastructure/persistence/model_versioning.py``,
``app/infrastructure/persistence/job_store.py``, ``api/middlewares/maintenance.py``) les implémente
via des adaptateurs
...
class PredictionPort(Protocol):
    """Contrat d'inférence de sentiment (cf. app/application/predictor_cache.get_predictor).
...
class ModelRepositoryPort(Protocol):
    """Contrat de consultation des versions de modèles
    (cf. app/infrastructure/persistence/model_versioning)."""
