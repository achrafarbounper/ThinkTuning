...
def test_build_core_openrouter_uses_key_and_url(monkeypatch) -> None:
    ...
    core = factory.build_agent_core()
    assert core._llm.url == get_agent_config().openrouter_url
    assert core._llm.model == get_agent_config().model_name
...
def test_build_core_respects_budget_settings(monkeypatch) -> None:
    ...
    # max_rounds est lu par run() depuis Intent, mais la config effective porte
    # la valeur (module IHM : base > env > défauts).
    assert get_agent_config().max_llm_rounds == 3
