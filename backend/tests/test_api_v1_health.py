...
def no_models(monkeypatch, tmp_path):
    ...
    empty_root.mkdir()
    monkeypatch.setattr("app.infrastructure.persistence.model_versioning.MODEL_ROOT", str(empty_root))
...
def one_model(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("app.infrastructure.persistence.model_versioning.MODEL_ROOT", str(root))
    monkeypatch.setattr("app.infrastructure.persistence.job_store.get_job_store", lambda: {})
    return version_dir
...
def test_v1_health_counts_only_running_jobs(monkeypatch):
    ...
    from app.domain.entities.models import JobStatus, TrainJob
    ...
    monkeypatch.setattr("app.infrastructure.persistence.job_store.get_job_store", lambda: store)
