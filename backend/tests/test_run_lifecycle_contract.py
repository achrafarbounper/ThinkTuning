...
introspection des constantes et signatures — sans ouvrir aucune base :
    ...
    - chaînes d'audit figées == constantes de ``app/infrastructure/persistence/audit_store.py`` ;
...
def test_audit_action_strings_match_legacy() -> None:
    ...
    from app.infrastructure.persistence.audit_store import ACT_APPROVAL as LEGACY_APPROVAL
    from app.infrastructure.persistence.audit_store import ACT_RUN as LEGACY_RUN
...
def test_core_status_maps_cover_all_run_statuses() -> None:
    from app.infrastructure.persistence import run_store as legacy
...
def test_legacy_run_store_signatures_match_typed_port() -> None:
    from app.infrastructure.persistence.run_store import RunStore

    _assert_signature_subset(RunStorePort, RunStore)


def test_legacy_approval_store_signatures_match_typed_port() -> None:
    from app.infrastructure.persistence.approval_store import ApprovalStore
