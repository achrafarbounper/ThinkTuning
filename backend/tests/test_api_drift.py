...
def test_drift_identical_distributions_no_alert(client, monkeypatch):
    ...
    texts_a = ["a", "b", "c", "d"]
    texts_b = ["a", "b", "c", "d"]
    resp = client.post("/api/v1/drift", json={"texts_a": texts_a, "texts_b": texts_b}, headers=HEADERS)
...
def test_drift_threshold_configurable(client, monkeypatch):
    ...
    low = client.post("/api/v1/drift", json={**body, "threshold": 0.01}, headers=HEADERS).json()
    high = client.post("/api/v1/drift", json={**body, "threshold": 10.0}, headers=HEADERS).json()
...
def test_drift_invalid_method(client, monkeypatch):
    _setup(monkeypatch)
    resp = client.post(
        "/api/v1/drift", params={"method": "js"}, json={"texts_a": ["a"], "texts_b": ["b"]}, headers=HEADERS
    )
...
def test_drift_invalid_threshold(client, monkeypatch):
    _setup(monkeypatch)
    resp = client.post(
        "/api/v1/drift", params={"threshold": 0}, json={"texts_a": ["a"], "texts_b": ["b"]}, headers=HEADERS
    )
    assert resp.status_code == 400
...
def test_drift_empty_batch(client, monkeypatch):
    _setup(monkeypatch)
    resp = client.post("/api/v1/drift", json={"texts_a": [], "texts_b": ["a"]}, headers=HEADERS)
    assert resp.status_code == 400


def test_drift_missing_fields(client, monkeypatch):
    _setup(monkeypatch)
    resp = client.post("/api/v1/drift", json={"texts_a": ["a"]}, headers=HEADERS)
    assert resp.status_code == 400
...
def test_drift_requires_api_key(client, monkeypatch):
    _setup(monkeypatch)
    resp = client.post("/api/v1/drift", json={"texts_a": ["a"], "texts_b": ["b"]})
...
def test_drift_chi2_single_label_reference(client, monkeypatch):
    ...
    assert resp.status_code == 400
    assert "khi-deux" in resp.json()["error"]["message"]
