...
dépendance ``_ = Depends(require_api_key)`` ; côté v1 l'auth est appliquée par
la route, puis la valeur factice ``True`` est transmise à l'appel direct (la
dépendance FastAPI n'est pas rejouable en appel de fonction).
...
def update_agent_settings(
    update: legacy.AgentSettingsUpdate, _: bool = Depends(require_api_key)
):
...
def test_agent_connectivity(
    request: legacy.ConnectivityTestRequest, _: bool = Depends(require_api_key)
):
...
def ask_core(request: legacy.AskRequest, _: bool = Depends(require_api_key)):
...
def ask_core_stream(
    request: legacy.AskStreamRequest, _: bool = Depends(require_api_key)
) -> StreamingResponse:
...
def multi_ask_stream(
    request: legacy.MultiAskRequest, _: bool = Depends(require_api_key)
) -> StreamingResponse:
...
def list_flow_sessions(
    limit: int = 50, status: str | None = None, _: bool = Depends(require_api_key)
):
