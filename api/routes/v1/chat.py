...
ex-``/ai``). Les deux endpoints exigent X-API-Key (parité legacy).
...
@router.get("/models")
def list_llm_models(_: bool = Depends(require_api_key)):
    """Liste les modèles LLM installés sur le serveur (sélecteur du chat)."""
    return _call_guarded(legacy.list_available_llm_models)


@router.post("/ai")
def ai_chat(request: legacy.ChatRequest, _: bool = Depends(require_api_key)) -> StreamingResponse:
